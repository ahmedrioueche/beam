...
const SignupForm: React.FC<SignupFormProps> = ({
  ...
  const handleSignup = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (email === "test@example.com" && password === "password" && password === confirmPassword) {
        onSignupSuccess();
      } else {
        onSignupFailure("Invalid email or passwords do not match");
      }
    }, 1500);
  };
      ...
      <div className="flex-1 flex md:items-center justify-center bg-white dark:bg-gray-800 p-8">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSignup} className="space-y-6">
            ...
            >
              {isLoading ? <FaSpinner className="animate-spin" /> : dict[selectedLanguage].signup}
            </button>
